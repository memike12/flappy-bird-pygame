...
def getReward(oldState, action, newState):
    try:
        reward = rewardTable[(oldState, action, newState)]
        return reward
    except KeyError: 
        return 0

def qLookup(state, action):
    try:
...
def main():
    ...
    while not finished:
        ...
        top_font = pygame.font.SysFont(None, 32, bold=True)  # default font
        bottom_font = pygame.font.SysFont(None, 32, bold=True)  # default font
        btop_font = pygame.font.SysFont(None, 32, bold=True)  # default font
        bbot_font = pygame.font.SysFont(None, 32, bold=True)  # default font
        pipe_font = pygame.font.SysFont(None, 32, bold=True)  # default font
        images = load_images()
        ...
        #print('pipe left: ' + str(pipes[0].rect[0]) + ' \tright: ' + str(pipes[0].rect[0] + 80) + ' \ttop: ' + str(p.top_height_px) + ' \tbottom: ' + str(WIN_HEIGHT - p.bottom_height_px))
